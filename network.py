...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        pos1_embedding = tf.get_variable('pos1_embedding', [settings.pos_num, settings.pos_size])#####创建一个名为pos_embedding，大小为【123，5】
        ...
        inputs_forward = tf.concat(axis=2, values=[tf.nn.embedding_lookup(word_embedding, self.input_word),
                                                   ...
                                                   tf.nn.embedding_lookup(pos2_embedding, self.input_pos2)])####从wordembedding中找到索引为inputword的向量，正向
        ...
        inputs_backward = tf.concat(axis=2,values=[tf.nn.embedding_lookup(word_embedding, tf.reverse(self.input_word, [1])),
                                                   ...
                                                   tf.nn.embedding_lookup(pos2_embedding,tf.reverse(self.input_pos2, [1]))])####反向，向量变成反向
        ...
        for i in range(big_num):
            ...
            with tf.name_scope("loss"):
                self.loss.append(
                    tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=sen_out[i], labels=self.input_y[i])))#####损失函数
                if i == 0:
            ...
            with tf.name_scope("accuracy"):
                self.accuracy.append(
                    tf.reduce_mean(tf.cast(tf.equal(self.predictions[i], tf.argmax(self.input_y[i], 0)), "float"),name="accuracy"))###tf.cast类型转换
            # with tf.name_scope("precision"):
            #     self.precision.append(
            #         tf.cast(tf.metrics.precision(self.predictions[i], tf.argmax(self.input_y[i], 0)), "float"))###tf.cast类型转换
            # with tf.name_scope("recall"):
            #     self.precision.append(
            #         tf.cast(tf.metrics.recall(self.predictions[i], tf.argmax(self.input_y[i], 0)),"float"))
        tf.summary.scalar('loss', self.total_loss)
            ...
            # regularization
        self.l2_loss = tf.contrib.layers.apply_regularization(regularizer=tf.contrib.layers.l2_regularizer(0.001), weights_list=tf.trainable_variables())
