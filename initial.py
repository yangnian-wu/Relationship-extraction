...
def init():
    ...
    relation2id = {}
    f = open('./datav2/relation2idv2.txt', 'r', encoding='utf-8-sig')  ####读取关系
    while True:
        content = f.readline()
        if content == '':
            break
    ...
    f = open(r'./datav2/trainv2.txt', 'r', encoding='utf-8')
    while True:
        ...
        if tup not in train_sen:
            ...
            y_id = relation
            label_tag = 0
            label = [0 for i in range(len(relation2id))]  #####把标签转化成one-hot
            label[y_id] = 1
            train_ans[tup] = []
            ...
            if temp == -1:
                train_ans[tup].append(label)  #####给tup实体对添加值，值是标签
        ...
        for i in range(min(fixlen, len(sentence))):
            ...
            output[i][0] = word
        train_sen[tup][label_tag].append(output)#{(),[字索引，位置值1，位置值2]}
    #同训练数据一样的处理方式
    ...
    f = open('./datav2/testv2.txt', 'r', encoding='utf-8')
    a=0
    while True:
        content = f.readline()
        a = a + 1
        ...
        if tup not in test_sen:
            ...
            label = [0 for i in range(len(relation2id))]
            label[y_id] = 1
            test_ans[tup] = label#test_ans:{(entity1,entity2):标签}
        else:
            y_id = relation
            test_ans[tup][y_id] = 1
        sentence = content[3]
        ...
        test_sen[tup].append(output)#{(实体1，实体2)，词嵌入+位置嵌入表示的句子}
    train_x = []
    train_y = []
    test_x = []
    test_y = []
...
def seperate():
    ...
    np.save('./data/train_word.npy', train_word)##实体
...
# get answer metric for PR curve evaluation获得PR曲线评估的答案指标，获得真实标签
def getans():
